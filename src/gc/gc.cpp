...
static int cache_node_cmp(const void *a, const void *b) {
    auto ca = (const struct cache_node *) a;
    auto cb = (const struct cache_node *) b;
...
static __inline void node_free(int id) {
    E.pool[id].mark = -1;
    if (E.pool[id].u.n.children != nullptr) {
...
void gc_exit() {
    ...
    for (i = 0; i < E.size; i++) {
        ...
        if (E.pool[i].mark >= 0) {
            ...
            if ((E.pool[i].u.n.finalizer != nullptr) && E.pool[i].u.c.weak != WEAK_CONTAINER) {
...
void gc_collect() {
    ...
    for (i = 0; i < E.size; i++) {
        if (E.pool[i].mark < E.mark) {
            ...
            if ((E.pool[i].u.n.finalizer != nullptr) && E.pool[i].u.c.weak != WEAK_CONTAINER) {
                E.pool[i].u.n.finalizer(p);
                E.pool[i].u.n.finalizer = nullptr;
            }
            my_free(p);
            map_erase(i);
...
void *gc_realloc(void *p, size_t sz, void *parent) {
    ...
    if (p == nullptr) {
        return gc_malloc(sz, parent, nullptr);
    }
